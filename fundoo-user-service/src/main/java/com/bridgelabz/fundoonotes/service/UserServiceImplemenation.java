...
//	RabbitMQSender rabbitMQSender;
	...
	public boolean register(RegisterDto userDto) {
		...
		if (fetchedUser == null) {
			...
			String emailBodyContaintLink = Util.createLink("http://localhost:8080/fundoo/user-service/user/verification",
	...
	public User login(LoginDto loginInformation) {
		...
		if (fetchedUser != null) {
			if (bCryptPasswordEncoder.matches(loginInformation.getPassword(), fetchedUser.getPassword())) {
				 ...
				 */
				String unVerifiedUsertosendmail = Util.createLink("http://localhost:8080/fundoo/user-service/user/verification",
		 ...
		 * user not found
		 */
		throw new UserNotFoundException("UserNotFound",HttpStatus.NOT_FOUND);
	}
	...
	public boolean isUserExist(String emailId) {
		...
		if (userdata != null) {
			...
			if (userdata.isVerified()) {
				 ...
				 */
				String emailLink = "http://localhost:8080/fundoo/user-service/user/forgotPassword"
...
//					" go check for login");
	...
	public User getUser(String token) {
		...
		User user = userRepository.getUser(userId);//redisRepository.findById(userId);
